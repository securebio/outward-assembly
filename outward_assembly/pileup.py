...
Usage:
    python -m outward_assembly.pileup --fwd-fastq reads_R1.fastq --rev-fastq reads_R2.fastq \\
        --contigs-fasta contigs.fasta --contig-name contig_name --seed-sequence seed_seq --output output.png
...
class PileupReadPairRow:
    ...
    insertion_positions: Set[int] = None

    def __post_init__(self):
        if self.insertion_positions is None:
...
DEFAULT_COLORS = {
    ...
    ),  # #4A90E2 - Blue (seed-containing reads)
    "match_no_seed": np.array(
        [140, 170, 210, 255], dtype=np.uint8
    ),  # Muted blue (non-seed reads)
    "seed": np.array([255, 140, 0, 255], dtype=np.uint8),  # #FF8C00 - Orange
    "seed_mismatch": np.array(
        [220, 40, 40, 255], dtype=np.uint8
    ),  # #DC2828 - Red (mismatch at seed position)
    "mismatch": np.array([220, 40, 40, 255], dtype=np.uint8),  # #DC2828 - Red
    "deletion": np.array([0, 0, 0, 255], dtype=np.uint8),  # Black
    "soft_clip": np.array(
        [100, 200, 220, 255], dtype=np.uint8
    ),  # Cyan -- this is basically an insertion at the edges, but good to have a separate color + CIGAR distinguishes them
    "insertion_marker": np.array(
        [255, 0, 255, 255], dtype=np.uint8
    ),  # Magenta - used in edge sidebars for insertion counts
    "insertion_stripe": np.array(
        [255, 105, 180, 255], dtype=np.uint8
    ),  # Hot pink - diagonal stripes for inline insertion markers
...
def run_bowtie2(
    ...
    if not os.path.exists(f"{index_base}.1.bt2"):
        ...
        subprocess.run(["bowtie2-build", "-q", contigs_fasta, index_base], check=True)
    ...
    if not os.path.exists(sorted_bam_path):
        ...
        subprocess.run(["samtools", "view", "-bS", "-o", bam_path, sam_path], check=True)

        # Sort BAM
        print("Sorting BAM...")
        subprocess.run(["samtools", "sort", "-o", sorted_bam_path, bam_path], check=True)
...
def find_seed_positions(contig_seq: str, seed_seq: str) -> Set[int]:
    ...
    if not positions:
        print("Warning: Seed sequence not found in contig")
...
def process_mate(
...
) -> Tuple[np.ndarray, int, int, int, bool, Set[int]]:
    """
    Convert one mate's alignment to a 2D array of pixel colors.
    ...
    # Estimate output size: query length + potential deletions
    estimated_size = read.query_length + sum(
        length for op, length in read.cigartuples if op == CIGAR_D
    )
    ...
    start_coord = ref_pos
    leading_soft_clip = None  # Store leading soft clip to prepend later
    ...
    for op_idx, (op, length) in enumerate(read.cigartuples):
        ...
        if op == CIGAR_M:  # aligned base
            ...
            insertions += length
            query_pos += length
            # Track the position just before the insertion for diagonal stripe marking
            if color_idx > 0:
                insertion_positions.add(color_idx - 1)

        elif op == CIGAR_D:  # Deletion (bases in the reference contig, but not in the read)
            color_array[color_idx : color_idx + length] = colors["deletion"]
            color_idx += length
            ref_pos += length

        elif op == CIGAR_S:  # Soft clip
            # Leading soft clip if first op, or only preceded by hard clips (e.g., 5H10S50M)
            is_leading = all(read.cigartuples[i][0] == CIGAR_H for i in range(op_idx))
            if is_leading:
                # Leading soft clip: bases are BEFORE reference_start
                start_coord = ref_pos - length
                leading_soft_clip = np.full(
                    (length, 4), colors["soft_clip"], dtype=np.uint8
                )
            else:
                # Trailing soft clip: bases are AFTER aligned region
                color_array[color_idx : color_idx + length] = colors["soft_clip"]
                color_idx += length
            query_pos += length
    ...
    color_array = color_array[:color_idx]

    # Prepend leading soft clip if present
    if leading_soft_clip is not None:
        # Shift insertion positions by the soft clip length
        soft_clip_len = len(leading_soft_clip)
        insertion_positions = {pos + soft_clip_len for pos in insertion_positions}
...
def adjust_colors_for_noseed(
    color_array: np.ndarray, colors: Dict[str, np.ndarray]
) -> np.ndarray:
    """
    Replace match_and_contains_seed color with match_no_seed color.

    Args:
        color_array: (N, 4) RGBA array
        colors: Color palette dict

    Returns:
        Modified color array (may be same array if no changes needed)
    """
    if len(color_array) == 0:
        return color_array
    match_color = colors["match_and_contains_seed"]
    noseed_color = colors["match_no_seed"]
    # Find rows matching the seed-containing color
    mask = np.all(color_array == match_color, axis=1)
    if np.any(mask):
        color_array = color_array.copy()
        color_array[mask] = noseed_color
    return color_array
...
def process_read_pair(
    ...
    # Adjust colors for non-seed-containing reads
    if not contains_seed:
        up_colors = adjust_colors_for_noseed(up_colors, colors)
        down_colors = adjust_colors_for_noseed(down_colors, colors)
    ...
    if len(up_colors) == 0:
        return PileupReadPairRow(
            ...
            insertion_positions=down_ins_pos,
        )
    if len(down_colors) == 0:
        return PileupReadPairRow(
            colors=up_colors,
            ...
            ins_downstream=0,
            contains_seed=contains_seed,
            insertion_positions=up_ins_pos,
        )

    # Combine mates with gap or overlap handling
    gap = down_start - up_end
    combined_ins_pos = set(up_ins_pos)  # Start with upstream positions

    if gap > 0:
        # Gap between mates - fill with unsequenced color
        gap_colors = np.full((gap, 4), colors["unsequenced"], dtype=np.uint8)
        combined = np.concatenate([up_colors, gap_colors, down_colors])
        # Downstream positions shift by upstream length + gap
        down_offset = len(up_colors) + gap
        combined_ins_pos.update(pos + down_offset for pos in down_ins_pos)
    elif gap < 0:
        # Mates overlap - skip overlapping portion of downstream
        overlap = -gap
        if overlap < len(down_colors):
            combined = np.concatenate([up_colors, down_colors[overlap:]])
            # Downstream positions shift, but only those past the overlap
            down_offset = len(up_colors) - overlap
            combined_ins_pos.update(
                pos + down_offset for pos in down_ins_pos if pos >= overlap
            )
        else:
            # Downstream entirely within upstream
            combined = up_colors
            # No downstream insertion positions to add
    else:
        # Exactly adjacent
        combined = np.concatenate([up_colors, down_colors])
        # Downstream positions shift by upstream length
        combined_ins_pos.update(pos + len(up_colors) for pos in down_ins_pos)
    ...
    return PileupReadPairRow(
        ...
        contains_seed=contains_seed,
        insertion_positions=combined_ins_pos,
    )
...
def get_read_pair_rows(
    ...
    with pysam.AlignmentFile(bam_path, "rb") as samfile:
        ...
        try:
            ...
            print("Warning: Could not fetch by region, scanning entire BAM")
    ...
    rows = []
    for mates in read_pairs.values():
        if len(mates) == 2:
...
class PileupRenderer:
    ...
    def render(
        ...
        # Create canvas
        canvas = np.full(
            (total_height, total_width, 4), self.colors["background"], dtype=np.uint8
        )
        ...
        canvas[
            contig_y_start:contig_y_end, contig_x_start : contig_x_start + contig_len
        ] = self.colors["match_and_contains_seed"]
        ...
        # Track insertion positions in canvas coordinates for stripe rendering
        insertion_stripe_positions = []  # List of (canvas_y, canvas_x)
        ...
        for i, row in enumerate(rows):
            ...
            # Track insertion positions for stripe rendering
            for ins_pos in row.insertion_positions:
                canvas_x = x_start + ins_pos
                if 0 <= canvas_x < total_width:
                    insertion_stripe_positions.append((y, canvas_x))
            ...
            if row.ins_downstream > 0:
                ...
                canvas[y, down_x_start : down_x_start + ins_count] = self.colors[
                    "insertion_marker"
                ]
        ...
        # Draw thin vertical line at right edge of insertion positions
        stripe_color = self.colors["insertion_stripe"]
        for canvas_y, canvas_x in insertion_stripe_positions:
            # Calculate scaled patch bounds
            scaled_y_start = canvas_y * scale[1]
            scaled_y_end = scaled_y_start + scale[1]
            scaled_x_start = canvas_x * scale[0]
            scaled_x_end = scaled_x_start + scale[0]

            # Draw a 1-pixel wide vertical line at the right edge of this base
            # (where the insertion starts)
            scaled[scaled_y_start:scaled_y_end, scaled_x_end - 1] = stripe_color
    ...
    def add_legend(self, image: np.ndarray) -> np.ndarray:
        ...
        legend_items = [
            ...
            ("Insertion begins", self.colors["insertion_stripe"]),
            ("Inserted bases", self.colors["insertion_marker"]),
        ...
        canvas = np.full(
            (new_height, img_width, 4), self.colors["background"], dtype=np.uint8
        )
...
def create_pileup_visualization(
    ...
    print("Step 1: Aligning reads to contigs...")
    ...
    print("Step 3: Finding seed positions...")
    ...
    print("Step 4: Parsing alignments...")
...
def main():
    ...
    parser.add_argument("--fwd-fastq", required=True, help="Forward reads FASTQ file")
    parser.add_argument("--rev-fastq", required=True, help="Reverse reads FASTQ file")
    parser.add_argument("--contigs-fasta", required=True, help="Contigs FASTA file")
    parser.add_argument("--contig-name", required=True, help="Name of contig to visualize")
    parser.add_argument("--seed-sequence", required=True, help="Seed sequence to highlight")
    ...
    parser.add_argument(
        "--contig-height", type=int, default=10, help="Contig ribbon height"
    )
